...
void EXTI9_5_IRQHandler() {
    ...
    if (S1.last_push_time + 15 > get_total_count()) return;

    S1.is_pressed = HAL_GPIO_ReadPin(S1_GPIO_Port, S1_Pin);

    if (S1.last_push_time + 1000 > get_total_count() && S1.is_pressed == 1) {
        next_song_display();
        S1.double_press_timeout = 0x7FFFFFFF;
        S1.activate_single_press = 0;
    } else if (S1.last_push_time + 1000 < get_total_count() && S1.is_pressed == 0) {
        LED_Status = 0;
        S1.double_press_timeout = 0x7FFFFFFF;
        S1.activate_single_press = 0;
